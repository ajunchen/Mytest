...
#include "gc0310mipi_yuv_Sensor_cmk.h"
#include "gc0310mipi_yuv_Camera_Sensor_para_cmk.h"
#include "gc0310mipi_yuv_CameraCustomized_cmk.h"
...
static UINT32 GC0310MIPIGetSensorID(UINT32 *sensorID)
{
    ...
    if(*sensorID != GC0310_SENSOR_ID_CMK) 
    {
        *sensorID = 0xFFFFFFFF; 
        SENSORDB("Read Sensor ID Fail blx 222= 0x%04x\n", *sensorID); 
        return ERROR_SENSOR_CONNECT_FAIL;
    }
	printk("mt_get_gpio_in get sensorid cmk= %d\n",__gpio_get_value(19));
    /*
    if (*sensorID != GC0310_SENSOR_ID) {
        *sensorID = 0xFFFFFFFF; 
        return ERROR_SENSOR_CONNECT_FAIL;
...
static UINT32 GC0310MIPIOpen(void)
{
	...
	if(sensor_id != GC0310_SENSOR_ID_CMK)  
	{
...
SENSOR_FUNCTION_STRUCT	SensorFuncGC0310MIPIYUVCMK=
{
	GC0310MIPIOpen,
	GC0310MIPIGetInfo,
	GC0310MIPIGetResolution,
...
UINT32 GC0310_YUV_SensorInit_CMK(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&SensorFuncGC0310MIPIYUVCMK;
	return ERROR_NONE;
} /* SensorInit() */
